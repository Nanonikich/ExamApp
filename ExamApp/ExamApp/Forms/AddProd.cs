...
namespace ExamApp
{
    public partial class AddProd : Form
    {
        ...
        private void EditData(byte[] arr, DB db)
        {
            ...
            //var i = 0;
            //string[,] znach = { { "@Vend", "@Photo", "@PhotoUrl", "@Product", "@Descr", "@Price", "@Categ" }, { textBoxVC.Text, arr.ToString(), imageUrl, textBoxNam.Text, textBoxDesc.Text, textBoxPr.Text, textBoxCat.Text } };
            //var cmd = new SqlCommand("INSERT INTO Products (prod_vendcode, prod_image, prod_imgUrl, prod_name, prod_descr, prod_price, prod_category) " +
            //                         "VALUES (@Vend, @Photo, @PhotoUrl, @Product, @Descr, @Price, @Categ)", db.GetConnection());
            //while (i != 7)
            //{
            //    cmd.Parameters.AddWithValue(znach[0, i], znach[1, i]);
            //    i += 1;
            //}
            //cmd.ExecuteNonQuery();
        }
